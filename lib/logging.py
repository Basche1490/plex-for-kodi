...
def log(msg, *args, **kwargs):
    ...
    xbmc.log('script.plexmod.zidoo: {0}'.format(msg), level)


def log_error(txt='', hide_tb=False):
    short = str(sys.exc_info()[1])
    if hide_tb:
        xbmc.log('script.plexmod.zidoo: ERROR: {0} - {1}'.format(txt, short), xbmc.LOGERROR)
        return short

    tb = traceback.format_exc()
    xbmc.log("_________________________________________________________________________________", xbmc.LOGERROR)
    xbmc.log('script.plexmod.zidoo: ERROR: ' + txt, xbmc.LOGERROR)
...
def service_log(msg, level=xbmc.LOGINFO, realm="Updater"):
    xbmc.log('script.plexmod.zidoo/{}: {}'.format(realm, msg), level)
