    ...
    def load(self):
        ...
        if cachexml_match:
            ...
            try:
                ...
                DEBUG_LOG("script.plexmod.zidoo: invalid or not found memorysize in advancedsettings.xml")

            try:
                self.readFactor = int(ADV_RFACT_RE.search(cachexml).group(1))
            except:
                DEBUG_LOG("script.plexmod.zidoo: invalid or not found readfactor in advancedsettings.xml")
