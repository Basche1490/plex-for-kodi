...
def disable_enable_addon():
    ...
    try:
        xbmc.executeJSONRPC(json.dumps({'jsonrpc': '2.0', 'id': 1, 'method': 'Addons.SetAddonEnabled',
                                 'params': {'addonid': 'script.plexmod.zidoo', 'enabled': False}}))
        xbmc.executeJSONRPC(json.dumps({'jsonrpc': '2.0', 'id': 1, 'method': 'Addons.SetAddonEnabled',
                                 'params': {'addonid': 'script.plexmod.zidoo', 'enabled': True}}))
    except:
        raise
                    ...
                    if update_version:
                        ...
                        if dir_loc:
                            ...
                            if dest_loc:
                                ...
                                if do_start:
                                    xbmc.executebuiltin('RunScript(script.plexmod.zidoo,0,0,1)')
