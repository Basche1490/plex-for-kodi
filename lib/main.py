...
def waitForThreads():
    ...
    if time.time() >= started + util.addonSettings.maxShutdownWait or (exit_timer_started and not exit_timer_was_alive):
        util.LOG('Main: script.plexmod.zidoo: threads took too long or timer hit, HARD EXITING')
        sys.exit(0)

@atexit.register
def realExit():
    xbmc.log('Main: script.plexmod.zidoo: REALLY FINISHED', xbmc.LOGINFO)
    if quitKodi:
        xbmc.log('Main: script.plexmod.zidoo: QUITTING KODI', xbmc.LOGINFO)
...
def hardExit():
    util.LOG('Main: script.plexmod.zidoo: timer hit, triggering hard exit...')
